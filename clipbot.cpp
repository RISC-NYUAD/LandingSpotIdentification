...
int main(int argc, char* argv[])
{
  ...
  co.Execute(solutionA, -7.0);

original.push_back(subj);
   
  //draw solution ...
 SVGBuilder svg1;
    svg1.style.penWidth = 0.8;
    svg1.style.pft = pftEvenOdd;
    svg1.style.brushClr = 0x1200009C;
    svg1.style.penClr = 0xCCD3D3DA;
    svg1.AddPaths(original);
    svg1.style.brushClr = 0x129C0000;
    svg1.style.penClr = 0xCCFFA07A;
    svg1.AddPaths(original);
    svg1.style.brushClr = 0x6080ff9C;
    svg1.style.penClr = 0xFF003300;
    svg1.style.pft = pftNonZero;
    svg1.AddPaths(solutionA);
    svg1.SaveToFile("solutionA.svg");

//v0.2 - OpenCV
Mat contourImage = imread("input/bw.png");
Mat img_gray;
cvtColor(contourImage, img_gray, COLOR_BGR2GRAY);
...
Mat image_copy1 = contourImage.clone();
drawContours(image_copy1, contours1, -1, Scalar(155, 255, 0), 2, LINE_AA);
drawContours(image_copy1, contours1, -1, Scalar(155, 255, 0), 3, LINE_AA);
	...
	for(int i=0;i<contours1.size();i++){//for(int i=0;i<contours1.size();i++){
		Path obstacles;
		for(int j=0;j<contours1[i].size();j++){
			obstacles << IntPoint(contours1[i][j].x,contours1[i][j].y); 
		}
	...
	//draw solution ...
 	SVGBuilder svg11;
    svg11.style.penWidth = 0.8;
    svg11.style.pft = pftEvenOdd;
    svg11.style.brushClr = 0x1200009C;
    svg11.style.penClr = 0xCCD3D3DA;
    svg11.AddPaths(border2);
    svg11.style.brushClr = 0x129C0000;
    svg11.style.penClr = 0xCCFFA07A;
    svg11.AddPaths(obstaclePaths);
    svg11.style.brushClr = 0x6080ff9C;
    svg11.style.penClr = 0xFF003300;
    svg11.style.pft = pftNonZero;
    svg11.AddPaths(solutionC);
    svg11.SaveToFile("solutionB.svg");
...
mapbox::geometry::polygon<double> polygonFULL;
	...
	for(int i=0;i<solutionC.size();i++){
		...
		for(int j=0;j<solutionC[i].size();j++){
		...
		//mapbox::geometry::point<double> pA = mapbox::polylabel(polygonFULL, 1.0);
